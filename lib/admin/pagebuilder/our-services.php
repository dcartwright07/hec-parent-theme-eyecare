...
function wc_kc_our_services_shortcode($atts){
	...
	if ($services_query->have_posts() ) {
		...
		while ($services_query->have_posts() ) {
			...
			// Check if user chooses to display services button
			if( get_field( 'services_button' ) == 'yes' ) {
				// Check if user inserted ID to append to URL
				if( get_field( 'services_link_id' ) ) {
					$output .= '<h4><a href="' . esc_url( get_field( 'services_link' ) ) . esc_html( get_field( 'services_link_id' ) ) . '" title="' . esc_html( get_the_title() ) . '">' . esc_html( get_the_title() ) . '</a></h4>';
					$output .= get_the_content();
					$output .= '<div class="content-button">';
					$output .= '<a href="' . esc_url( get_field( 'services_link' ) ) . esc_html( get_field( 'services_link_id' ) ) . '" target="_self" title="' . esc_html( get_the_title() ) . '">Read more</a>';
					$output .= '</div>';
				} else {
